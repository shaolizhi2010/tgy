...
public class PanSearchLocalCacheService {
	...
	public static void work( ){
		...
		try {
			...
			String keywordJson = Connecter.connect(" http://www.webhezi.com/needDigKeywords.jsp", "utf-8");
			 
			List<SearchHistory> keywords =  new Gson().fromJson(keywordJson,new TypeToken<List<SearchHistory>>(){}.getType());
			
			for(SearchHistory sh : keywords ){
	...
	public static void dig(String keyword){
		...
		try {
			...
			for (Page p : list) { //www.webhezi.com
				...
				String connectUrl = "http://www.webhezi.com/createCache.jsp?name="
